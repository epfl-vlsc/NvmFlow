...
/*
std::pair<std::string, bool> parseAnnotation() {}

void insertValidII(Instruction* i, InstructionInfo::InstructionType instrType) {
  auto* ii = getII(i);
  if (!ii) {
    return;
  }

  if (auto [annot, hasAnnot] = isAnnotatedField(ii, FIELD_ANNOT); hasAnnot) {
    // find current
    auto [st, idx] = getFieldInfo(ii);
    auto strIdx = StructElement::getAbsoluteName(st, idx);
    auto* seCurrent = units.dbgInfo.getStructElement();

    units.variables.insertVariable(current);

    // find pair
    auto [pairStr, useDcl] = parseAnnotation;
  }
